...
//   execute the command
//   respond on stdout
int fromCDT(STATE *pstate, const char *line, int linesize)			// from cdt
{
    ...
    else if (strcmp(cc.argv[0], "-gdb-set") == 0)
    {
        ...
        target.BreakpointCreateForException(lldb::LanguageType::eLanguageTypeC_plus_plus, true, true);
    ...
    else if (strcmp(cc.argv[0], "-exec-run") == 0)
    {
        ...
        logprintf(LOG_NONE, "launchInfo: args=%d env=%d, pwd=%s\n", launchInfo.GetNumArguments(), launchInfo.GetNumEnvironmentEntries(),
                launchInfo.GetWorkingDirectory());
        SBError error;
            ...
            else if (strcmp(cc.argv[nextarg], "kill") == 0)
            {
                if (process.IsValid())
                {
                    ...
                    if (process.IsValid())
                    {
                        int state = process.GetState();
                        if (state == eStateStopped)
                        {			// if process is stopped. restart it before kill
                            SBThread thread = process.GetSelectedThread();
                            cdtprintf("%d^running\n", cc.sequence);
                            cdtprintf("*running,thread-id=\"%d\"\n(gdb)\n", thread.IsValid() ? thread.GetIndexID() : 0);
                            process.Continue();
                ...
                cdtprintf("%d^error,msg=\"%s\"\n(gdb)\n", cc.sequence, "Command unimplemented1.");
        }
    }
    // BEAKPOINT COMMANDS
    else if (strcmp(cc.argv[0], "-break-insert") == 0)
    {
        ...
        if (breakpoint.IsValid())
        {
            ...
            cdtprintf("%d^done,bkpt=%s\n(gdb)\n", cc.sequence, formatBreakpoint(breakpointdesc, sizeof(breakpointdesc), breakpoint, pstate));
        }
        else
            cdtprintf("%d^error\n(gdb)\n", cc.sequence);
    }
    ...
    else if (strcmp(cc.argv[0], "-list-thread-groups") == 0)
    {
        ...
        else if (cc.argv[nextarg] == NULL)
        {
            ...
            if (filename != NULL && filedir != NULL)
                snprintf(groupsdesc + groupdesclength, sizeof(groupsdesc) - groupdesclength, ",executable=\"%s/%s\"", filedir, filename);
        ...
        else if (strcmp(cc.argv[nextarg], "sharedlibrary") == 0)
        {
            ...
            for (int iModule = 0; iModule < nModules; iModule++)
            {
                ...
                if (module.IsValid())
                {
                    ...
                    cdtprintf("~\"0x%016x\t0x%016x\t%s\t\t%s/%s\"\n", addrLoadS, addrLoadS + addrLoadSize, moduleHasSymbols, moduleFilePath, moduleFileName);
                    ++nLibraries;
                }
            }
            if (nLibraries == 0)
            ...
            cdtprintf("%d^error,msg=\"%s\"\n(gdb)\n", cc.sequence, "Command unimplemented2.");
    }
    else if (strcmp(cc.argv[0], "-data-list-register-names") == 0)
    {
        ...
        cdtprintf("%d^error,msg=\"%s\"\n(gdb)\n", cc.sequence, "Command unimplemented3.");
    }
    else
    {
...
bool addEnvironment(STATE *pstate, const char *entrystring)
{
    ...
    if (pstate->envpentries >= ENV_ENTRIES - 2)
    {		// keep size for final NULL
...
int scanArgs(CDT_COMMAND *cc)
{
    ...
    while (*pa)
    {
        if (cc->argc >= MAX_ARGS - 2)
        {		// keep place for final NULL
